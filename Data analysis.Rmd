...
```{r}
# 202330BCEPR_stom
my_dat %>% filter(trial_name == "202330BCEPR_stom") %>% 
...
ggsave(paste0("images\\germ_stom", trial_interest, "_", ".png"),
        units = "in", dpi = 300, width = 15, height = 6
   )


# 202331BCEPR_cere
my_dat %>% filter(trial_name == "202331BCEPR_cere") %>% 
  ggplot(aes(x = reorder(accession_name, -germination_perc), 
             y = germination_perc, fill = factor(rep_number))) +
  facet_grid(factor(rep_number)~trial_name) +
  geom_col(position = "dodge") +
...
my_dat %>% select(trial_name, rep_number, accession_name, germination_perc) %>%
  filter(germination_perc < 50, trial_name == "202330BCEPR_stom") %>% arrange(germination_perc) %>% pivot_wider(names_from = rep_number, values_from = germination_perc)
```
...
index_traits <- c("DM_gravity", "plant_type", "yield_ha", "germination_perc")
...
res.pca <- pca_index(data = index_dat, id = "accession_name", b = c(10, -5, 10, 10), percentage = 0.50)
